...
const activePredios = computed(() => catalog.predios.filter(
  (item) => item.activo === true && (!review.value?.cliente_id || item.cliente_id === Number(review.value.cliente_id)),
))
  ...
  <main class="mx-auto max-w-md overflow-x-hidden p-4 pb-24 text-gray-900 dark:text-gray-100">
    ...
    <form v-else-if="state === 'reviewing' || state === 'confirming'" class="space-y-4" @submit.prevent="confirmReview">
      ...
      <section class="space-y-4 rounded-xl bg-white p-4 shadow dark:bg-gray-800">
        ...
        <label class="block text-xs font-medium text-gray-600 dark:text-gray-300">Predio<select v-model.number="review.predio_id" required aria-label="Predio" class="mt-1 min-h-11 w-full rounded-lg border p-2 dark:border-gray-600 dark:bg-gray-700"><option :value="null">Seleccionar predio</option><option v-for="predio in activePredios" :key="predio.id" :value="predio.id">{{ predio.descripcion }}</option></select></label>
