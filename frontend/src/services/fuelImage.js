...
const FUEL_IMAGE_MAX_BYTES = 10 * 1024 * 1024
const FUEL_IMAGE_TYPES = new Set(['image/jpeg', 'image/png', 'image/webp'])
const FUEL_IMAGE_EXTENSION = /\.(?:jpe?g|png|webp)$/i
const FUEL_IMAGE_RESIZE_MAX_SIDE = 2200
const FUEL_IMAGE_RESIZE_QUALITY = 0.82

export const FUEL_TIPO_TICKET = 'ticket'
export const FUEL_TIPO_REMITO_INTERNO = 'remito_interno'

export const validateFuelImageFile = (file, maxBytes = FUEL_IMAGE_MAX_BYTES) => {
  if (typeof Blob === 'undefined' || !(file instanceof Blob)) throw new TypeError('Seleccioná una imagen válida.')
  const type = String(file.type || '').trim().toLowerCase()
  const name = typeof file.name === 'string' ? file.name.trim() : ''
  const supported = type ? FUEL_IMAGE_TYPES.has(type) : FUEL_IMAGE_EXTENSION.test(name)
...
export const prepareFuelImageForUpload = async (file, {
  maxBytes = FUEL_IMAGE_MAX_BYTES,
  maxSide = FUEL_IMAGE_RESIZE_MAX_SIDE,
  quality = FUEL_IMAGE_RESIZE_QUALITY,
...
export const createFuelImageLifecycle = () => {
  let active = true
  return {
...
export const analyzeFuelImage = async (file, tipo, httpClient = axios, apiUrl = API_URL) => {
  if (typeof Blob === 'undefined' || !(file instanceof Blob)) throw new TypeError('Seleccioná una imagen válida.')
  if (tipo !== FUEL_TIPO_TICKET && tipo !== FUEL_TIPO_REMITO_INTERNO) {
    throw new TypeError('Tipo de comprobante inválido.')
  }
  const body = new FormData()
  const filename = typeof file.name === 'string' && file.name.trim() ? file.name.trim() : 'image'
  body.append('file', file, filename)
  body.append('tipo', tipo)
  const response = await httpClient.post(endpoint(apiUrl, '/fuel-image/analyze'), body)
  return response.data
}

export const confirmFuelTicketImage = async (payload, httpClient = axios, apiUrl = API_URL) => {
  const response = await httpClient.post(endpoint(apiUrl, '/fuel-image/confirm/ticket'), payload)
  return response.data
}

export const confirmFuelRemitoInternoImage = async (payload, httpClient = axios, apiUrl = API_URL) => {
  const response = await httpClient.post(endpoint(apiUrl, '/fuel-image/confirm/remito-interno'), payload)
  return response.data
}

export const fuelImageUrl = (id, apiUrl = API_URL) => {
  if (!Number.isInteger(id) || id <= 0) throw new TypeError('El id de imagen debe ser un entero positivo.')
  return endpoint(apiUrl, `/fuel-image/${encodeURIComponent(String(id))}/blob`)
}
...
export const fetchFuelImageBlob = async (id, httpClient = axios, apiUrl = API_URL) => {
  const response = await httpClient.get(fuelImageUrl(id, apiUrl), { responseType: 'blob' })
  if (!isImageBlob(response?.data)) throw new TypeError('La respuesta no contiene una imagen válida.')
  return response.data
}

export const createFuelImageObjectUrl = (blob, urlApi = URL) => {
...
export const revokeFuelImageObjectUrl = (url, urlApi = URL) => {
...
export const mapFuelImageError = (error) => {
